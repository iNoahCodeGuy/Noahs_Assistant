...
def _build_self_knowledge_chunk() -> dict:
    ...
    return {
        "content": (
            ...
            "data capture.\n\n"
            "DESIGN TRADEOFFS I CAN DISCUSS IN DEPTH:\n"
            "- Why pgvector over Pinecone: one service, one bill, one failure point. Pinecone is "
            "overkill for ~200 chunks. IVFFLAT handles this dataset in single-digit milliseconds.\n"
            "- Why functional pipeline over LangGraph StateGraph: same node contract (partial dict "
            "returns), but a for loop replaced the framework orchestrator. Error traces point at "
            "Python functions, not framework internals. No conditional edges needed — linear path "
            "with short-circuit exits.\n"
            "- Why Claude Haiku for classification over regex: regex breaks on semantic ambiguity. "
            "'yo what is this thing' is a greeting with zero greeting keywords. Haiku costs ~0.1 "
            "cents per call and eliminates an entire category of misrouting bugs.\n"
            "- What I would change: streaming responses (perceived latency near zero), hybrid "
            "search (keyword + vector for exact term matches), smarter memory that weights topics "
            "by engagement signals instead of recency.\n\n"
            "FAILURE MODES I CAN EXPLAIN:\n"
            "- Retrieval returns nothing: dual thresholds (0.50 strict, 0.30 fallback), then "
            "grounding validation, then graceful fallback or self-knowledge injection.\n"
            "- LLM hallucinates: hallucination_check() compares output against retrieved chunks. "
            "System would rather say less than say something wrong.\n"
            "- External service down: no fallback model (intentional — voice consistency matters "
            "more than uptime). Twilio failure is silent, generation failure is honest.\n"
            "- Hostile/inappropriate input: deflect without drama, offer architecture explanation "
            "instead. Boundaries without performance.\n\n"
            "DEVELOPMENT STORY:\n"
            "- Built iteratively over several weeks. First version: FAISS + GPT-4o, basic RAG.\n"
            "- Migrations: FAISS → pgvector (serverless-compatible), GPT-4o → Claude Sonnet "
            "(better voice adherence).\n"
            "- Hardest part: getting the voice right. System prompt went through dozens of "
            "revisions. You cannot unit test personality.\n"
            "- Knowledge base is fully curated (~200 chunks), not scraped. Every chunk was "
            "written by Noah. Coverage is limited but accuracy is high.\n\n"
